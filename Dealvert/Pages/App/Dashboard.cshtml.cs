...
public class DashboardModel : PageModel
{
    ...
    public string PlanDescription { get; private set; } = "Start with 5 free active alerts.";
        ...
        CreditPurchase? latestPaidUnlimited)
    {
        ...
        if (IsUnlimitedPlan)
        {
            ...
            PlanDescription = PlanDaysRemaining is null
                ? "Unlimited active alerts and reports."
                : $"Unlimited active alerts and reports until {PlanExpiresOnUtc!.Value.ToLocalTime():MMMM d, yyyy}.";
        ...
        PlanDescription = limits.Capacity <= 5
            ? "Free credits to validate price alerts before paying."
            : "Active capacity. Add packs when you want to watch more products.";
        PlanCapacityLabel = $"{limits.Capacity} active alert{(limits.Capacity == 1 ? "" : "s")}";
